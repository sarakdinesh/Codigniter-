...
class Howto extends Admin_Controller {
    ...
    public function index() {
        $this->load->view('back-office-panel/how_to');
    }

    public function add($id) {
        if (isset($id)) {
            $data["datas"] = $this->master->get_by_column("unique_id", $id);
            $this->load->view('back-office-panel/add_how_to', $data);
        } else {
            $this->load->view('back-office-panel/add_how_to');
        }
    }
    ...
    public function lists() {
        ...
        foreach ($list as $master) {
            ...
            $url = base_url() . "howto/" . replaceAll($master->page_name) . "/" . $master->unique_id;
...
//add html for action
            $row[] = '<a class="btn-floating waves-effect waves-light purple lightrn-1" href="' . base_url('back-office/how-to-add/') . $master->unique_id . '" '
                    . 'title="Edit">

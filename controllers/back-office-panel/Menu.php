...
class Menu extends Admin_Controller {
    ...
    public function index() {
        $this->load->view('back-office-panel/menu');
    }

    public function add($id) {
        if (isset($id)) {
            $data["datas"] = $this->master->get_by_column("unique_id", $id);
            $this->load->view('back-office-panel/add_menu', $data);
        } else {
            $this->load->view('back-office-panel/add_menu');
        }
    }
...
//add html for action
            $row[] = '<a class="btn-floating waves-effect waves-light purple lightrn-1" href="' . base_url('back-office/menu-add/') . $master->unique_id . '" '
                    . 'title="Edit">
...
//output to json format
    ...
    public function save() {
        ...
        if ($id != "") {
            ...
            redirect('back-office-panel/menu');
        } else {

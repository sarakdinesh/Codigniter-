...
class Homespecial extends Admin_Controller {
    ...
    public function index() {
        $this->load->view('back-office-panel/homespecial');
    }

    public function add($id) {
        if (isset($id)) {
            $data["datas"] = $this->master->get_by_column("unique_id", $id);
            $this->load->view('back-office-panel/add_homespecial', $data);
        } else {
            $this->load->view('back-office-panel/add_homespecial');
        }
    }
    ...
    public function lists() {
        ...
        foreach ($list as $master) {
            ...
            $url = base_url() . "special/" . replaceAll($master->page_name) . "/" . $master->unique_id;
...
//add html for action
            $row[] = '<a class="btn-floating waves-effect waves-light purple lightrn-1" href="' . base_url('back-office/homespecial-add/') . $master->unique_id . '" '
                    . 'title="Edit">
...
//output to json format
    ...
    public function save() {
        ...
        $unique_id = UniqueID();
        $this->table = "tbl_home_special_category";
        $menus = $this->master->get_by_id($this->input->post('menu_id'));
        $this->table = "tbl_home_special";
        if ($id != "") {
            if (empty($_FILES['image']['name'])) {
                ...
                $data = array(
                    ...
                    'main_category' => $menus->main_category,
                ...
                redirect('back-office-panel/homespecial');
            } else {
                $this->upload->initialize(set_upload_options());

                $this->image_moo->load($_FILES['image']['name'])->resize(350, 350)->save("uploads/123.jpg", TRUE);
//                $this->image_moo->load($temp_file)
//                        ->crop(20, 20, 80, 80)
//                        ->load_temp()
//                        ->resize(100, 100)
//                        ->save("result.jpg", true);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($_FILES['image']['name'])) {
                    ...
                    if (!$this->upload->do_upload("image")) {
                        ...
                        //// this is very imp for image manipulate
                        $this->image_moo->load($_FILES['image']['tmp_name'])->resize(350, 270)->save("uploads/tumbnail/" . $imgname . "", TRUE);
                        //////
                        ...
                        $this->session->set_flashdata('success', 'Data Saved Successfully.');
                        redirect('back-office/homespecial');
                    }
                }
            }
        }
